// Reverse Order
export const cards = [
  {
    title: "iCalc",
    section: "Experiments",
    description:
      "A calculator app built with vanilla JavaScript and HTML. Based on Apple's old iOS calculator.",
    tags: ["JavaScript", "HTML", "CSS"],
    live: "https://bonevane.github.io/calculator/",
    code: "https://github.com/Bonevane/calculator",
    thumbnail: "./projects/Calculator.webp",
  },
  {
    title: "X&O",
    section: "Experiments",
    description:
      "A tic-tac-toe game built to demonstrate basic game logic and UI design.",
    tags: ["JavaScript", "HTML", "CSS"],
    live: "https://bonevane.github.io/tic-tac-toe/",
    code: "https://github.com/Bonevane/tic-tac-toe",
    thumbnail: "./projects/TicTacToe.webp",
  },
  {
    title: "To Do",
    section: "Experiments",
    description:
      "A simple to-do list app built with vanilla JavaScript and HTML.",
    tags: ["JavaScript", "HTML", "CSS"],
    live: "https://bonevane.github.io/to-do/",
    code: "https://github.com/Bonevane/to-do",
    thumbnail: "./projects/ToDo.webp",
  },
  {
    title: "Weather",
    section: "Experiments",
    description:
      "A weather app that fetches data from Visual Crossing API. Based on Pixel Weather from Google. Not quite responsive yet. ",
    tags: ["API", "JS", "Visual Crossing"],
    live: "https://bonevane.github.io/weather-app/",
    code: "https://github.com/Bonevane/weather-app",
    thumbnail: "./projects/Weather.webp",
  },
  {
    title: "Autolist",
    section: "Experiments",
    description:
      "An inventory system containing detailed info about cars from the GTA series...Well some of them.",
    tags: ["Express.js", "PostgreSQL", "Neon DB"],
    live: "https://gta-cars-inventory.onrender.com/manufacturers",
    code: "https://github.com/Bonevane/gta-cars-inventory   ",
    thumbnail: "./projects/Autolist.webp",
  },
  {
    title: "Notebox",
    section: "Experiments",
    description: "A mini message board app built using Express.js.",
    tags: ["Express.js", "EJS", "Forum"],
    live: "https://mini-message-board-mdy9.onrender.com/",
    code: "https://github.com/Bonevane/mini-message-board",
    thumbnail: "./projects/Messages.webp",
  },
  {
    title: "Inside",
    section: "Game_Dev",
    description:
      "A minimalist, narrative-heavy 2D text adventure. Inspired by acclaimed titles like To the Moon, Signalis, and Undertale.",
    tags: ["Narrative", "Indie", "C++", "SFML"],
    live: "https://bonevane.itch.io/inside",
    code: "https://github.com/Bonevane/Inside-Remastered",
    thumbnail: "./projects/Inside.webp",
  },
  {
    title: "Orrery",
    section: "Game_Dev",
    description:
      "A 2D real-time planetary sandbox that visualizes orbital mechanics, collisions, and other dynamic systems.",
    tags: ["Java", "LibGDX", "Physics", "Space"],
    live: "https://bonevane.github.io/Orrery/",
    code: "https://github.com/Bonevane/Orrery",
    thumbnail: "./projects/Orrery.webp",
  },
  {
    title: "Path Tracer",
    section: "Game_Dev",
    description:
      "An elementary path tracer that simulates realistic lighting and material interactions in Unity.",
    tags: ["Unity", "HLSL", "C#"],
    live: "",
    code: "https://github.com/Bonevane/Path-Tracer",
    thumbnail: "./projects/PathTracer.webp",
  },
  {
    title: "Aperture",
    section: "Game_Dev",
    description:
      "A 2D Hyper-Casual Puzzle game built in Unity. Based on “Portal” from Valve.",
    tags: ["Unity", "Android", "Puzzle", "2D"],
    live: "https://bonevane.itch.io/aperture",
    code: "",
    thumbnail: "./projects/FeatureLarge.webp",
  },
  {
    title: "Never Settle",
    section: "Blender",
    description:
      "A 3D model of the OnePlus 8 made as a homage to my old favorite.",
    tags: ["Oneplus", "Product Render", "3D"],
    live: "",
    code: "",
    thumbnail: "./projects/Oneplus.webp",
  },
  {
    title: "Living",
    section: "Blender",
    description:
      "A study of lighting and materials inside a lived in space. Made in Blender.",
    tags: ["Interior", "Materials", "Lighting"],
    live: "",
    code: "",
    thumbnail: "./projects/Couch.webp",
  },
  {
    title: "Formula 1 Car",
    section: "Blender",
    description:
      "A high poly blender model of a Formula 1 car, based on the 2022 regulations.",
    tags: ["3D", "F1", "Cycles", "Materials"],
    live: "",
    code: "",
    thumbnail: "./projects/F1.webp",
  },
  {
    title: "Sunset Drive",
    section: "Blender",
    description: "An experiment in sunset & nature composition in Blender.",
    tags: ["Blender", "3D", "Realism", "Nature"],
    live: "",
    code: "",
    thumbnail: "./projects/SunsetDrive.webp",
  },
  {
    title: "Danish's Space",
    section: "Web_Dev",
    description:
      "A portfolio website for Danish Munib, showcasing his work and skills. Built with Vanilla JS and CSS.",
    tags: ["JavaScript", "HTML", "CSS", "Frontend", "Design"],
    live: "https://bonevane.github.io/homepage/ ",
    code: "",
    thumbnail: "./projects/dmunib.webp",
  },
  {
    title: "GHMS",
    section: "Web_Dev",
    description:
      "An hospital management system designed to manage patient records, appointments, and staff schedules at Gulgasht Hospital Multan.",
    tags: ["PostgreSQL", "Next.js", "Healthcare"],
    live: "",
    code: "https://github.com/Bonevane/GHMS",
    thumbnail: "./projects/GHMS.webp",
  },
  {
    title: "INF-19",
    section: "Web_Dev",
    description:
      "An interactive infectious disease sandbox for a synthetic social network, with configurable parameters for both the network structure and disease characteristics.",
    tags: ["D3.js", "Network Sim", "Data Viz"],
    live: "https://inf-19.vercel.app",
    code: "https://github.com/Bonevane/INF-19",
    thumbnail: "./projects/INF-19.webp",
  },

  {
    title: "Spillage",
    section: "Web_Dev",
    description:
      "A search engine for over 190k indexed Medium articles. Built on Google's foundational research paper.",
    tags: ["Next.js", "FastAPI", "NLP", "DSA"],
    live: "https://spillage-search.vercel.app",
    code: "https://github.com/Bonevane/Spillage-Search",
    thumbnail: "./projects/Spillage.webp",
  },
];

export const colors = {
  Home: ["#FFA73C", "#FFA73C", "#FF3232"], // Orange

  Highlights: ["#FFE29A", "#F4B63F", "#5A3A00"], // Gold
  Web_Dev: ["#00E6E6", "#0073CB", "#001F3F"], // Blue
  Blender: ["#00B4DB", "#00C2CB", "#005f73"], // Cyan
  Experiments: ["#00B4DB", "#7449FF", "#005f73"], // Purple
  Game_Dev: ["#FFF9C4", "#FFC01F", "#FBC02D"], // Yellow
  Photos: ["#222222", "#222222", "#222222"], // Dark Gray
  Skills: ["#222222", "#444444", "#222222"], // Dark Gray

  Contact: ["#A2DED0", "#16A085", "#004D40"], // Teal
  Miscellaneous: ["#FFADC0", "#FF3C73", "#A00048"], // Pink

  // Additional (Unused)
  Red: ["#00E6E6", "#5A0C0D", "#001F3F"], // Dark Red
  Contact5: ["#5227FF", "#7cff67", "#5227FF"], // Green
  OrangeL: ["#ff5f6d", "#ffc371", "#ff5f6d"], // Light Orange
  YellowL: ["#FF8A88", "#FFFA80", "#FF8A80"], // Light Yellow
};
